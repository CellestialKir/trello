...
export const addColumn = (title, boardId) => async (dispatch) => {
  const response = await fetch("https://682dc1864fae18894757672d.mockapi.io/colums", {
    method: "POST",
    headers: {
...
export const deleteColumn = (id) => async (dispatch) => {
  const response = await fetch(`${"https://682dc1864fae18894757672d.mockapi.io/colums"}/${id}`, {
      method: "DELETE",
  });
...
export const reorderColumns = (reorderedColumns) => async (dispatch) => {
  ...
  try {
    ...
    for (let i = 0; i < reorderedColumns.length; i++) {
      ...
      await fetch(`${"https://682dc1864fae18894757672d.mockapi.io/colums"}/${column.id}`, {
        method: "PUT",
        headers: {
          "Content-Type": "application/json",
        },
